...
5. **Proactive context surfacing** — before diving into new work, run the linked-refs skill to surface relevant past notes about any people, projects, or topics mentioned. E.g. if Andrew mentions "Barelang", search memory for all prior mentions. If you're starting a project, check for prior decisions in `~/proactivity/session-state.md`.
...
## 🗺️ Memory Tools (Obsidian-inspired)

Andrew has access to a set of memory tools inspired by Obsidian's PKM system:

**Available skills:**
- `daily-template` — Auto-generate structured daily note with context, agenda, loose ends, and review sections. Run on every new session.
- `linked-refs` — Search all memory/notes for mentions of a person, project, or topic. Use proactively before starting new work.
- `memory-graph` — Generate an interactive D3.js force-directed graph of all notes. Shows connections between people, projects, and topics visually.
- `atomic-notes` — Refactor bloated notes into one-idea-per-note atomic units, linked together.
- `publish-note` — Convert any `.md` note to a standalone HTML page and publish it as a public GitHub Pages URL.

**When to use proactively:**
- Before working on a new project → run `linked-refs` to recall prior context
- Every session start → ensure today's daily note exists via `daily-template`
- When notes feel messy → run `atomic-notes` refactor
- Andrew asks to "see connections" → run `memory-graph`
- Andrew wants to share a note → run `publish-note`
